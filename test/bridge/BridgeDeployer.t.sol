...
contract BridgeDeployerTest is Test {
    ...
    function setUp() public {
        _bridgeDeployer = new BridgeDeployer{salt: CREATE_BRIDGE_DEPLOYER_SALT}(address(this));
    }

    function test_should_deploy_bridge() public {
        address expectedAddress = _bridgeDeployer.bridgeAddress();
        address bridgeAddress = _bridgeDeployer.deploy(FOUNDRY_BID, address(0xb0b));
    ...
    function test_should_bridge_deployment_address_be_deterministic_regarding_owner() public {
        address expectedAddress = _bridgeDeployer.bridgeAddress();
