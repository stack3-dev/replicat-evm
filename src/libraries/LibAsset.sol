...
library LibAsset {
    ...
    function hash(Asset memory asset) internal pure returns (bytes32) {
        return keccak256(abi.encode(uint8(asset.type_), asset.chainBid, asset.address_, asset.metadata));
    }

    /// @notice Compute the replica address using CREATE3
    /// @dev using asset hash as salt
    /// @param asset the asset data
    function computeReplicaAddress(Asset memory asset) internal view returns (address) {
    ...
    function computeReplicaAddress(bytes32 assetHash) internal view returns (address) {
